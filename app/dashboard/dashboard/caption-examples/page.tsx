    ...
    return (
      <>
        <div style={{ marginBottom: 14 }}>
          ...
          <textarea value={data.caption || ''} onChange={e => onChange('caption', e.target.value)} rows={3} style={{ width: '100%', resize: 'vertical' }} />
        </div>
        <div style={{ marginBottom: 14 }}>
          <label style={labelStyle}>IMAGE DESCRIPTION</label>
          <textarea value={data.image_description || ''} onChange={e => onChange('image_description', e.target.value)} rows={3} style={{ width: '100%', resize: 'vertical' }} />
        </div>
        <div style={{ marginBottom: 14 }}>
          <label style={labelStyle}>EXPLANATION</label>
          <textarea value={data.explanation || ''} onChange={e => onChange('explanation', e.target.value)} rows={2} style={{ width: '100%', resize: 'vertical' }} />
        </div>
        <div style={{ display: 'flex', gap: 16, marginBottom: 14 }}>
          ...
          <div style={{ flex: 2 }}>
            ...
            <input value={data.image_id || ''} onChange={e => onChange('image_id', e.target.value)} style={{ width: '100%' }} placeholder="Optional" />
          </div>
        </div>
      </>
    )
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      <PageHeader title="Caption Examples" subtitle="Training examples for the LLM pipeline" count={rows.length}
        action={<button onClick={() => { setForm(blank); setShowCreate(true) }} style={{ padding: '8px 18px', background: 'var(--accent)', border: 'none', borderRadius: 7, color: '#fff', fontWeight: 500, fontSize: 13 }}>+ New Example</button>}
      />
