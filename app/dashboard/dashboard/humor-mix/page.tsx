...
import { updateFields } from '@/lib/db-helpers'
          ...
          <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 10, padding: '16px 20px', marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', height: 24, borderRadius: 6, overflow: 'hidden', gap: 2 }}>
              {mix.map((m, i) => {
                ...
                return <div key={m.id} title={`${m.humor_flavors?.slug}: ${m.caption_count}`} style={{ width: `${pct}%`, background: colors[i % colors.length], minWidth: 4 }} />
              })}
            </div>
                ...
                {mix.map(m => (
                  <tr key={m.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 12, color: 'var(--accent)' }}>{m.humor_flavors?.slug || `#${m.humor_flavor_id}`}</span>
                    </td>
                    <td style={{ padding: '12px 16px', fontSize: 13, color: 'var(--text2)', maxWidth: 300 }}>{m.humor_flavors?.description?.slice(0, 80) || '—'}</td>
                    <td style={{ padding: '12px 16px' }}>
                      <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 14, fontWeight: 600, color: 'var(--text)', background: 'var(--bg3)', border: '1px solid var(--border)', borderRadius: 6, padding: '4px 12px' }}>×{m.caption_count}</span>
                    </td>
                    <td style={{ padding: '12px 16px' }}>
                      <button onClick={() => setEditRow({ ...m })} style={{ padding: '5px 14px', background: 'transparent', border: '1px solid var(--border2)', borderRadius: 5, color: 'var(--text2)', fontSize: 12 }}>Edit</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {editRow && (
        <Modal title="Edit Humor Mix Entry" onClose={() => setEditRow(null)} width={400}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <input type="number" min={0} max={20} value={editRow.caption_count} onChange={e => setEditRow((r: any) => ({ ...r, caption_count: e.target.value }))} style={{ width: '100%' }} />
          </div>
