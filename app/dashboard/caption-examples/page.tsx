...
export default function AllowedDomainsPage() {
  ...
  async function handleCreate() {
    ...
    setSaving(true)
    const { error } = await supabase.from('allowed_signup_domains').insert({ apex_domain: domain.trim().toLowerCase(), ...(await insertFields()) })
    setSaving(false)
  ...
  async function handleUpdate() {
    setSaving(true)
    const { error } = await supabase.from('allowed_signup_domains').update({ apex_domain: editRow.apex_domain.toLowerCase(), ...(await updateFields()) }).eq('id', editRow.id)
    setSaving(false)
    if (error) { alert(error.message); return }
  ...
  const columns = [
    ...
    { key: 'apex_domain', label: 'DOMAIN', render: (v: string) => <span style={{ fontFamily: 'IBM Plex Mono, monospace', fontSize: 13, color: 'var(--green)' }}>@{v}</span> },
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      <PageHeader title="Allowed Signup Domains" subtitle="Only emails from these domains can sign up" count={rows.length}
        action={<button onClick={() => { setDomain(''); setShowCreate(true) }} style={{ padding: '8px 18px', background: 'var(--accent)', border: 'none', borderRadius: 7, color: '#fff', fontWeight: 500, fontSize: 13 }}>+ Add Domain</button>}
      />
      ...
      {showCreate && (
        <Modal title="Add Allowed Domain" onClose={() => setShowCreate(false)} width={400}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
            ...
            <button onClick={handleCreate} disabled={saving || !domain.trim()} style={{ padding: '8px 20px', background: 'var(--accent)', border: 'none', borderRadius: 6, color: '#fff', fontWeight: 500, opacity: !domain.trim() ? 0.5 : 1 }}>{saving ? 'Saving...' : 'Add'}</button>
          </div>
        </Modal>
      )}
      {editRow && (
      ...
      {deleteRow && (
        <Modal title="Remove Domain" onClose={() => setDeleteRow(null)} width={380}>
          <p style={{ color: 'var(--text2)', marginBottom: 20 }}>Remove <span style={{ fontFamily: 'IBM Plex Mono, monospace', color: 'var(--green)' }}>{deleteRow.apex_domain}</span>? New users from this domain will no longer be able to sign up.</p>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
